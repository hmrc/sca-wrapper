...
class WrapperService @Inject()(ptaMenuBar: PtaMenuBar,
  ...
  private def getWrapperDataResponse(request: Request[_]): Option[WrapperDataResponse] = {
    ...
    if (result.isEmpty) {
      logger.warn(s"[SCA Wrapper Library][WrapperService][getWrapperDataResponse]{ Expecting Wrapper Data in " +
  ...
  private def getMessageDataFromRequest(request: Request[_]): Option[Int] = {
    ...
    if (result.isEmpty) {
      logger.warn("[SCA Wrapper Library][WrapperService][getMessageDataFromRequest] Expecting Message Data in " +

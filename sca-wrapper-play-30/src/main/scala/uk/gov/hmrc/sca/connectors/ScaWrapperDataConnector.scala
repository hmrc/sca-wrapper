...
class ScaWrapperDataConnector @Inject() (http: HttpClient, appConfig: AppConfig) extends Logging {
  ...
  def wrapperData()(implicit
    ...
    http
      .GET[WrapperDataResponse](
        s"${appConfig.scaWrapperDataUrl}/wrapper-data?lang=$lang&version=${appConfig.versionNum}"
      )
  ...
  def messageData()(implicit ec: ExecutionContext, hc: HeaderCarrier): Future[Option[Int]] = {
    ...
    http
      .GET[HttpResponse](s"${appConfig.scaWrapperDataUrl}/message-data")
      .map { response =>

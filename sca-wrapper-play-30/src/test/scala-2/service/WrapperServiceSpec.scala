...
class WrapperServiceSpec extends BaseSpec {
  ...
  override def beforeEach(): Unit = {
    ...
    when(mockWebchatClient.loadRequiredElements()(any())).thenReturn(Some(Html("required elements")))
    when(mockWebchatClient.loadHMRCChatSkinElement(any())(any())).thenReturn(Some(Html("chat skin elements")))
  }
  ...
  "WrapperService" must {
    ...
    "return default New Sca layout with UR banner link from new data response" in {
      ...
      backLinkUrlCaptor.getValue mustBe None
      showSignOutInHeaderCaptor.getValue mustBe false
      scriptsCaptor.getValue mustBe List(Html("required elements"), Html("chat skin elements"))
      styleSheetsCaptor.getValue mustBe Seq.empty
      bannerConfigCaptor.getValue mustBe BannerConfig(
        showAlphaBanner = true,
        showBetaBanner = false,
    ...
    "return the exitSurveyOrigin if the continueUrl is None" in {
      ...
      val response =
        new WrapperService(mockPtaMenuBar, mockScaLayout, mockStandardScaLayout, mockWebchatClient, appConfig)
          .safeSignoutUrl(None)

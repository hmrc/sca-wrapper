...
//
//  def signOut(wrapperDataResponse: WrapperDataResponse): Action[AnyContent] = Action { implicit request =>
//    Redirect(config.ggSignoutUrl, Map("continue" -> Seq(config.ex))).withNewSession
//  }
//
